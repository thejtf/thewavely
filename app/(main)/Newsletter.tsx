  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-xs text-zinc-600 dark:text-zinc-400 md:text-sm">
        ...
        {subCount && (
          <span>
            已经有 <span className="font-medium">{subCount}</span> 位同学加入了，
          </span>
        )}
        <span>限时免费，每周一封。</span>
      </p>
        ...
        {!isSubscribed ? (
          ...
          >
            <input
              ...
              aria-label="电子邮箱"
              required
              className="min-w-0 flex-auto appearance-none rounded-lg border border-zinc-900/10 bg-white px-3 py-[calc(theme(spacing.2)-1px)] placeholder:text-zinc-400 focus:border-sky-500 focus:outline-none focus:ring-4 focus:ring-sky-500/10 dark:border-zinc-700 dark:bg-zinc-700/[0.15] dark:text-zinc-200 dark:placeholder:text-zinc-500 dark:focus:border-sky-400/50 dark:focus:ring-sky-400/5 sm:text-sm"
              {...register('email')}
            />
            <Button
              type="submit"
